...
namespace InstamojoAPI
{
     ...
     */
        ...
        */
        ...
        #region "get Payment Detail By Payment ID"

        public async Task<GetPaymentDetails> GetpaymentdetailAsync(string PaymentID)
        {
            if (PaymentID == null)
            {
                throw new ArgumentNullException(typeof(string).Name, "Transaction Id Can not be Null or Empty");
            ...
            try
            {
                string stream = api_call("GET", "payments/" + PaymentID + "/", null);
                GetPaymentDetails objPaymentRequestDetailsResponse = JsonConvert.DeserializeObject<GetPaymentDetails>(stream);
                return objPaymentRequestDetailsResponse;
            }
            catch (IOException ex)
            {
